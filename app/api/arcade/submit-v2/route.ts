...
type ArcadeGameKey = "nine" | "bigfish" | "clock";
...
function normGame(v: any): ArcadeGameKey | null {
  const g = String(v || "").trim().toLowerCase();
  if (g === "nine" || g === "bigfish" || g === "clock") return g;
  return null;
}

/* [HELP:ARCADE:API:SUBMIT_V2] START */
export async function POST(req: Request) {
  try {
    // Kun kiosk/ansvarlig må submitte (eventcode må ikke være offentlig)
    const kiosk = getKioskFromRequest(req);
    if (!kiosk) return bad("Unauthorized", 401);

    const body = await req.json().catch(() => ({}));

    const nickname = clampNickname(body?.nickname);
    const game = normGame(body?.game) || "nine";
